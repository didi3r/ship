...
readMore.directive('readMore', function() {
    return {
        ...
        template:  "<p> {{text | readMoreFilter:[text, countingWords, textLength] }}" +
            "<a ng-show='showLinks' ng-click='changeLength()' class='readmore'>" +
            " <span ng-show='isExpanded'>Mostrar menos</span>" +
            " <span ng-show='!isExpanded'>Mostrar más</span>" +
            "</a>" +
            "</p>",
        controller: ['$scope', '$attrs', '$element',
            function($scope, $attrs) {
                if($scope.text === undefined || $scope.text === null) return;
